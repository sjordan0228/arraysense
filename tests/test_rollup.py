...
def test_minute_bucket_averages_and_timestamps_at_minute_start() -> None:
    ...
    for sec, power in ((61, 10), (62, 20), (63, 30)):
        conn.execute(
            "INSERT INTO inverter_raw (timestamp, pv_total_power_w) VALUES (?, ?)",
            (sec, power),
        )
...
def test_max_and_min_metrics_keep_extremes_not_means() -> None:
    conn = _open()
    conn.execute("INSERT INTO serials (id, serial) VALUES (1, 'CE12345678')")
    ...
    for sec, cyc, vmin, soc in (
        ...
        conn.execute(
            "INSERT INTO module_raw (timestamp, module_id, cycle_count, "
            "cell_min_voltage_v, soc_pct) VALUES (?, 1, ?, ?, ?)",
            (sec, cyc, vmin, soc),
        )
...
def test_last_metric_takes_the_latest_value_not_a_mean() -> None:
    ...
    for sec, num in ((3601, 5), (3602, 7), (3603, 6)):
        conn.execute(
            "INSERT INTO module_raw (timestamp, module_id, cell_max_voltage_num) VALUES (?, 1, ?)",
            (sec, num),
        )
    rebuild_module_hourly(conn, 3600, 7200)
...
def test_sample_count_reflects_source_rows_covered() -> None:
    ...
    conn.execute("INSERT INTO inverter_raw (timestamp, pv_total_power_w) VALUES (0, 5)")
    conn.execute("INSERT INTO inverter_raw (timestamp, pv_total_power_w) VALUES (1, 7)")
    conn.execute("INSERT INTO inverter_raw (timestamp, pv_total_power_w) VALUES (60, 9)")
...
def test_a_bucket_whose_source_rows_are_gone_is_removed() -> None:
    conn = _open()
    conn.execute("INSERT INTO inverter_raw (timestamp, pv_total_power_w) VALUES (61, 10)")
    rebuild_inverter_minute(conn, 60, 120)
...
def test_failed_polls_are_excluded_from_the_aggregate() -> None:
    ...
    conn.execute(
        "INSERT INTO inverter_raw (timestamp, pv_total_power_w, error) VALUES (61, 10, NULL)"
    )
    conn.execute(
        "INSERT INTO inverter_raw (timestamp, pv_total_power_w, error) VALUES (62, 20, NULL)"
    )
    conn.execute(
        "INSERT INTO inverter_raw (timestamp, pv_total_power_w, error) "
        "VALUES (63, NULL, 'inverter unreachable')"
    )
    rebuild_inverter_minute(conn, 60, 120)
...
def test_module_rollups_group_by_module_not_by_slot() -> None:
    conn = _open()
    conn.execute("INSERT INTO serials (id, serial) VALUES (1, 'CE11111111')")
    conn.execute("INSERT INTO serials (id, serial) VALUES (2, 'CE22222222')")
    ...
    for sec, soc in ((3601, 80), (3602, 82), (3603, 81)):
        conn.execute(
            "INSERT INTO module_raw (timestamp, module_id, soc_pct) VALUES (?, 1, ?)",
            (sec, soc),
        )
    for sec, soc in ((3601, 50), (3602, 52)):
        conn.execute(
            "INSERT INTO module_raw (timestamp, module_id, soc_pct) VALUES (?, 2, ?)",
            (sec, soc),
        )
    rebuild_module_hourly(conn, 3600, 7200)
    rows = conn.execute(
...
def test_hourly_mean_ignores_rows_that_did_not_report_a_metric() -> None:
    ...
    for sec, soc in ((61, 80), (62, 82)):
        conn.execute(
            "INSERT INTO inverter_raw (timestamp, battery_soc_pct) VALUES (?, ?)",
            (sec, soc),
        )
    for sec in (121, 122, 123):
        conn.execute("INSERT INTO inverter_raw (timestamp) VALUES (?)", (sec,))
    rebuild_inverter_hourly(conn, 0, 3600)
...
def test_hourly_mean_averages_only_the_rows_that_report_a_metric() -> None:
    ...
    conn.execute("INSERT INTO inverter_raw (timestamp, pv_total_power_w) VALUES (61, 0)")
    conn.execute("INSERT INTO inverter_raw (timestamp) VALUES (62)")
    conn.execute("INSERT INTO inverter_raw (timestamp, pv_total_power_w) VALUES (121, 10)")
    rebuild_inverter_hourly(conn, 0, 3600)
...
def test_hourly_mean_rounds_instead_of_truncating() -> None:
    ...
    conn.execute("INSERT INTO inverter_raw (timestamp, pv_total_power_w) VALUES (61, 10)")
    conn.execute("INSERT INTO inverter_raw (timestamp, pv_total_power_w) VALUES (62, 11)")
    rebuild_inverter_hourly(conn, 0, 3600)
...
def test_negative_timestamp_bucket_floors_and_rebuild_is_idempotent() -> None:
    ...
    conn.execute("INSERT INTO inverter_raw (timestamp, pv_total_power_w) VALUES (-1, 5)")
...
def test_last_metric_keeps_a_value_reported_earlier() -> None:
    ...
    conn.execute(
        "INSERT INTO module_raw (timestamp, module_id, cell_max_voltage_num) VALUES (3601, 1, 5)"
    )
    conn.execute(
        "INSERT INTO module_raw (timestamp, module_id, cell_max_voltage_num) VALUES (3602, 1, 7)"
    )
    conn.execute("INSERT INTO module_raw (timestamp, module_id) VALUES (3603, 1)")
    rebuild_module_hourly(conn, 3600, 7200)
    row = conn.execute("SELECT cell_max_voltage_num FROM module_hourly").fetchone()
    conn.close()
    assert row is not None
