...
class SqliteStore:
    ...
    One instance owns one connection. Production gives the collector and API
    separate instances pointed at the same WAL database, so a slow raw-sample
    commit cannot hold the connection mutex needed by an API read. Writes go to
